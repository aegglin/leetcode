...
SELECT req.requester_id AS id, IFNULL(num_req + num_acc, num_req) AS num
FROM 
    (SELECT requester_id, COUNT(requester_id) AS num_req
        FROM request_accepted
        GROUP BY requester_id) AS req
    LEFT JOIN 
    (SELECT accepter_id, COUNT(accepter_id) AS num_acc
        FROM request_accepted
